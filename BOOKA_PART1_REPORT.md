...
### `.github/workflows/booka-part1-build.yml` (push/PR/workflow_dispatch)
1. 체크아웃, JDK 17 설치, Android SDK 설치
2. `gradle-wrapper.jar` 존재 확인 (없으면 즉시 실패 + bootstrap 워크플로 실행 안내)
3. `chmod +x ./gradlew` 및 실행권한 확인
4. `./gradlew clean` (로그: `01_clean.log`)
5. `./gradlew test` (로그: `02_test.log`, test 결과 xml/html 별도 수집)
6. `./gradlew lint` (로그: `03_lint.log`, lint 결과 html/xml/sarif 별도 수집)
7. `./gradlew assembleDebug` (로그: `04_assembleDebug.log`)
8. `app/build/outputs/apk/debug/app-debug.apk` 확인 후 `BOOKA_PART1-debug.apk`로 복사
   (경로가 다르면 즉시 오류로 실패 처리, 조용히 넘어가지 않음)
9. 체크아웃된 소스로 `BOOKA_Android_PART1.zip` 재생성(빌드 산출물·`.git` 제외)
10. `BOOKA_PART1-debug.apk`와 `BOOKA_Android_PART1.zip` 각각의 SHA-256 계산 →
    `BOOKA_PART1_SHA256SUMS.txt`
11. `01~04` 로그 + test/lint 결과를 묶어 `BOOKA_PART1_build_logs.zip` 생성
12. 6종 Artifact 업로드(`BOOKA_PART1-debug-apk`, `BOOKA_PART1_build_logs`,
    `BOOKA_PART1_test_results`, `BOOKA_PART1_lint_results`, `BOOKA_PART1_SHA256SUMS`,
    `BOOKA_Android_PART1_source`) — 실패해도 `if: always()`로 그때까지의 로그는 최대한 업로드.

두 워크플로 모두 `python3 -c "import yaml; yaml.safe_load(open('...'))"`로 파싱 검증했다
(각각 6개, 23개 step 정상 파싱). **다만 GitHub Actions 러너에서 실제로 실행해 성공한 적은
아직 없다.** 이 환경에는 GitHub Actions 자체가 없어 실행할 수 없기 때문이다.
